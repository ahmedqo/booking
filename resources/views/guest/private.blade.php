...
@section('title', ucwords(__('private excursions')) . ' | ' . __('Morocco Adventure City'))

@section('meta')
    <meta name="description" content="{!! __(
        'Discover exclusive private excursions in Morocco, offering personalized experiences in stunning locations like the Sahara Desert and Marrakech.',
    ) !!}">
    ...
    <meta property="og:title" content="{{ ucwords(__('private excursions')) . ' | ' . __('Morocco Adventure City') }}">
    <meta name="twitter:title" content="{{ ucwords(__('private excursions')) . ' | ' . __('Morocco Adventure City') }}">

    <meta property="og:image" content="{{ asset('img/slides/slide-3.webp') }}">
    <meta name="twitter:image" content="{{ asset('img/slides/slide-3.webp') }}">

    <script type="application/ld+json">
        {
            "@context": "https://schema.org",
            "@type": "WebPage",
            "name": "{{ __('Private Excursions') }}",
            "url": "{{ route('views.guest.private') }}",
            ...
            "offer": {
                "@type": "OfferCatalog",
                "name": "{{ __('Private Excursions') }}",
                "itemListElement": [@foreach (Core::trips(true) as $trip){
                        "@type": "Offer",
                        "name": "{!! $trip->title !!}",
                        "url": "{{ $trip->url }}",
                        "priceCurrency": "EUR",
